...
void rb_init(char_ringbuffer_t* buffer, uint_least8_t* data, uint_least8_t size)
{
	assert(buffer != 0);
	assert(data != 0);
	assert(size > 0);
...
void rb_put_data_blocking(char_ringbuffer_t* buffer, uint_least8_t data)
{
	...
	buffer->write_index = (buffer->write_index+1) & (buffer->mask);
}

void rb_put_data_nonblocking(char_ringbuffer_t* buffer, uint_least8_t data)
{
	assert(buffer != 0);
...
uint_least8_t rb_get_data_blocking(char_ringbuffer_t* buffer, uint_least8_t *data)
{
	assert(buffer != 0);
	assert(data != 0);
	...
	if (buffer->read_index != buffer->write_index)
	{
		*data = buffer->data[buffer->read_index];
		...
		// assuming that CHAR_RINGBUFFER_SIZE is a power of 2, the and operation
		// against CHAR_RINGBUFFER_SIZE-1 equals a modulo CHAR_RINGBUFFER_SIZE
		buffer->read_index = (buffer->read_index+1) & (buffer->mask);
...
uint_least8_t rb_get_data_nonblocking(char_ringbuffer_t* buffer, uint_least8_t *data)
{
	assert(buffer != 0);
	assert(data != 0);
